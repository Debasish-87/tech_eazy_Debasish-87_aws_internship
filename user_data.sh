...
# Clean up leftover AWS install dir
rm -rf /tmp/aws
...
echo "[INFO] Setting JAVA permissions and environment..."
...
echo "[INFO] Cloning Spring Boot app..."
...
echo "[INFO] Starting the Spring Boot application..."
...
aws s3 cp "$APP_READY_PATH" "s3://${BUCKET_NAME}/status/${ENVIRONMENT}/app_ready.txt" || {
  echo "[ERROR] Failed to upload app_ready.txt. Uploading failure marker..."
